    ...
    F: FnMut(&str) + Send + 'static + Clone,
{
    ...
    let (mut install_cmd, pre_cmd) = match distro.as_str() {
        "debian" => (vec!["sudo", "apt-get", "install", "-y"], Some(vec!["sudo", "apt-get", "update"])),
        "arch" => (vec!["sudo", "pacman", "-S", "--noconfirm"], Some(vec!["sudo", "pacman", "-Sy"])),
    ...
    if let Some(cmd) = pre_cmd {
        ...
        if !res.success {
            log(&format!("Warning: Package list update returned non-zero. Error: {}", res.stderr));
        }
    }
    ...
    for c in components {
        match c.id.as_str() {
            "base-deps" => {
                match distro.as_str() {
                    "debian" => pkgs.extend_from_slice(&["build-essential", "curl", "wget", "git", "unzip", "tar"]),
                    "arch" => pkgs.extend_from_slice(&["base-devel", "curl", "wget", "git", "unzip", "tar"]),
                    "redhat" => pkgs.extend_from_slice(&["gcc", "gcc-c++", "make", "curl", "wget", "git", "unzip", "tar"]),
                    _ => {}
                }
            }
            "tmux" => pkgs.push("tmux"),
    ...
    if result.success {
        ...
        Err(format!("Failed to install system packages. See detailed log."))
    }
}
